---
layout: single
title: 트러블슈팅 - 유지보수 어려운 예외 구조, 팀의 예외 생성 규칙을 정립하여 해결!
---
...
* ErrorCode Enum의 상수 100+ → 도메인별 분리
* 중복 예외 클래스 32% 감소, 신규 예외 생성 시 의사결정 시간 단축
* DynamicErrorCode로 런타임 메시지 지원
* "구조는 스스로를 말해야 한다"는 개발 철학
...
* ErrorCode Enum의 비대화
    * Nickname 형식이 유효하지 않네. -> (ErrorCode 탐색 후) 관련 상수가 없네. 추가해야겠다. -> (며칠 후) ...ErrorCode 하단에 이미 있었네? <br>
