...
## 왜 N+1 해결이 중요한가?
* 커뮤니티 웹사이트 특성 상 <u>다중 JOIN이 동반된 읽기 작업 및 집계 쿼리</u>의 비중이 큰 상황
...
* **높은 쿼리 통제권**
  * ORM 기반 연관관계 탐색 구조에서는 부모 엔티티 조회 이후 연관 엔티티 접근 시점마다 추가 쿼리가 발생할 수 있는 구조였음
  * jOOQ 도입 후 필요한 데이터를 JOIN 쿼리로 직접 구성하여 조회 쿼리의 실행 횟수를 예측 가능하게 통제할 수 있게 됨
* **쿼리 표현력 상승**
  * ORM의 지연 로딩에 의존하던 구조 탈피 -> 의도치 않은 N+1 쿼리 발생 가능성 제거
  * 데이터 접근 로직의 성능 특성을 코드 레벨에서 명확하게 이해 가능
* **개발 생산성 향상**
  * jOOQ가 JSONB를 직접 지원 -> 네이티브 쿼리 의존도 감소 + JSONB와 JsonNode 사이를 용이하게 변환 가능
  * DB 특화 기능을 활용하면서도 ORM 환경에서 필요한 커스텀 타입 처리 및 매핑 로직 제거

<br>

****

# 🌿 회고 : 문제가 발생할 수 있는 구조를 만들지 말라

예전에 백엔드 개발자 커뮤니티에서 주니어 개발자가 "N+1을 JPA로 어떻게 해결하나요?"라고 질문한 적이 있다. <br>
많은 현업 백엔드 개발자들께서 답변을 해주셨다. 그 중에서 오랜 경력과 실력으로 특히 주목받던 고참 개발자 님이 계셨는데, 그 분이 계신 회사에서 jOOQ를 사용한다며 하신 말씀이 있다.

<br>

> ORM 기반 영속성 프레임워크로 복잡한 집계 쿼리를 할 수는 있는데, 쿼리가 어려워지고 유지보수성이 나빠집니다. 저는 신입이었을 때 부터 "문제가 발생할 상황 자체를 만들지 말라"고 역설해왔습니다.

<br>

당시의 나는 그 말의 의미를 알지 못했다. 시간이 지나 N+1 문제를 마주한 지금, 그 말이 어떤 의미인지 알 듯 하다.

<br>

국내에서 N+1을 해결하는 대안 중 가장 널리 쓰이는 해법이 앞서 다뤘던 fetch join과 QueryDSL이다. 이 둘은 모두 **ORM 방식을 기반으로 한다**는 공통점이 있다. <br>
ORM은 데이터의 무결성과 일관성을 유지하는 데 강력한 도구다. 하지만 그 강점의 기반인 *영속성 컨텍스트와 완성된 엔티티 중심* 구조는 다중 JOIN 등 복잡한 조회나 집계 쿼리에서는 오히려 제약이 될 수 있다.

<br>

ORM의 설계 철학은 "필요한 데이터만 정확하게 조회하는 것"이 아니다. "엔티티를 어떻게 로딩할 것인가"이다. 결국 문제는 ORM 자체가 아니라 <u>목적의 성격과 맞지 않는 방식으로 접근하는 방식</u>에 있다.

<br>

**어디에서나 써도 좋은 만능 도구는 없다.** 도구는 목적에 맞게 사용하는 것이다. <br>
그리고 내린 결론은 다음과 같다: 데이터의 변경과 일관성이 중요한 작업에는 JPA/Hibernate를, 복잡한 조회와 집계 쿼리에는 jOOQ를 쓴다.
