...
* **DDD**
    ...
    * 도메인 클래스에 상태 관리 기능 추가 (ValidationService가 도메인 객체에 흡수됨)
...
* **오케스트레이션을 수행하는 Adapter 계층**
    ...
    * 그러나 애플리케이션의 대부분의 비즈니스 로직이 검증 및 단순한 DB 쿼리 수행 위주임
    * 비즈니스 로직이 단순하므로 개별적인 use case 클래스의 필요성 X
    ``` java
...
### 수치로 측정한 성과
* **측정 대상:** 댓글 삽입 API, 댓글 수정 API.
    * 두 API의 개발 난이도가 유사함. (CRUD 기능)
    * 비즈니스 로직 난이도가 쉬움 -> 아키텍처의 구조가 생산성에 끼치는 영향력 극대화 효과.
* **측정 범위:** RestController ~ Repository까지 API 전 단계 개발 후 테스트 완료 시점.
