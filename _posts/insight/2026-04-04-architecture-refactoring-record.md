...
👉 해당 아키텍처 리팩토링은 팀원들과 함께 진행함 <br>
다만, 이 글에서 정리한 문제 인식 / 해결 방향 / 아키텍처에 대한 해석은
리팩토링 과정에서 혼자 고민하고 정리한 사고 과정을 기반으로 작성함.
...
* 애플리케이션의 서버가 1대이고, 도메인들이 동일한 DB에 접근한다. 동일한 JPA 엔티티를 공유해도 되지 않을까? <br>

Bounded Context를 유지하려고 나누는 것 vs 전역적으로 공유하여 관리할 클래스의 수를 줄이는 것 사이의 고민이 있다.
...
* Rich Domain Model 채택 시, 도메인 클래스 내에 행동이 모두 들어가게 되므로 -> use case로 비즈니스 로직을 관리하는 Clean Architecture와 부자연스럽게 통합되리라 예상됨
...
아키텍처는 사람의 몸으로 비유하자면 "척추"다. 척추는 몸 전체를 관통하며, 어느 뼈가 어디부터 연결되어야 하는지를 정의한다. <br>
이 말을 다르게 하자면, 척추가 뒤틀리면 온 몸의 뼈도 같이 틀어진다는 의미다. 클래스 간 커플링과 경계가 모호하여 의존성이 뒤섞이는 과정 - 이를 프로그래밍 용어로 technical debt(기술적 부채)라고 한다.

<br>

아키텍처로 인한 기술적 부채는 **복리**로 늘어난다는 특징이 있다. 하나의 클래스가 잘못되면 다른 클래스들도 영향을 받는다. <br>
가령 Service들이 서로를 과하게 참조하면 "의존성을 받는 Service"와 "의존성을 제공하는 Service" 모두가 기술적 부채가 된다. 의존성을 1, 2개만 받는 게 아니라면 문제는 더욱 커질 것이다.

<br>

아키텍처를 리팩토링하며 치열하게 고민했다. <br>
완벽한 아키텍처도, 유일한 정답도 아니다. <br>
다만 확실한 건 - 이전에 사용하던 구조에 비해 유지보수성과 확장성이 늘어났다는 것. 아키텍처를 이해하느라 소요되는 기간이 단축되었다는 분명한 효과이다.

<br>

이번 아키텍처 리팩토링은 애플리케이션에 있어 전체적인 구조의 중요성을 다시금 깨닫는 시간이었다. 다시 아키텍처를 리팩토링할 때가 온다면, 애플리케이션의 맥락 + 아키텍처의 이론적 배경 + 미래의 확장성을 고려한 최선의 선택을 내릴 것이다.
