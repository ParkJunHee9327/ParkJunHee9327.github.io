...
* **측정 횟수**
    * 초기 DB 캐싱 및 오버헤드의 영향을 제거하기 위해 워밍업 구간(첫 2회 실행값) 제거
    * 이후 10번 반복 측정
...
#### 인덱스 설계 근거
* <u>복합 인덱스(post_ulid, created_at):</u> 게시글로 필터링 된 댓글의 정렬 최적화
* <u>단일 인덱스(post_ulid):</u> DB 옵티마이저가 상황에 따라 정렬 전용 인덱스를 사용하거나 Index Merge Scan을 활용하여 최적의 경로를 찾을 수 있을 것이라 예상
...
* **부차적 병목 지점 개선:** 최종 결과를 산출하기까지의 총 소요 시간 단축
