...
* **문제 상황:** 식물 커뮤니티 특성상 댓글 조회가 집중되는 구조에서 post_ulid 기반 필터링 및 created_at 기준 정렬을 최적화할 인덱스의 부재로 해결 우선순위가 높은 상황
* **해결 방안:** 복합 인덱스(post_ulid, created_at)를 설계하여 데이터 탐색과 정렬 비용을 동시에 개선하는 가설 수립 및 검증.
* **성과:** 평균 응답 속도 약 45% 개선(282ms → 143ms) 및 정렬 수행 시간 약 50% 단축 확인.

<br>

# 🧐 문제: 댓글 조회 API의 쿼리 구조에 최적화된 인덱스 부재

## 🩺 문제 상황: 인덱스 부재
* 해당 API는 화면에 표시되는 특정 게시글의 하단에 댓글 및 관련 정보를 표시하는 조회 API임
* 댓글 정보 + 댓글 작성자 + 작성자 프로필 + 댓글 좋아요의 정보를 동반하는 **다중 JOIN 구조**
...
* 해당 API가 개발된 프로젝트는 식물 커뮤니티로, 게시글 및 댓글 도메인의 비중이 큼
* 특히 게시글 및 게시글에 등록된 댓글을 조회하는 작업의 횟수가 많은 구조
* 댓글 수 증가 시 응답 지연이 발생할 가능성 존재 -> 위험을 사전에 방지하기 위해 성능 개선 진행

<br>

## 🚀 가설 및 테스트 방향성
* 커뮤니티 서비스 특성 상 <u>특정 게시글에 댓글이 집중되는 상황</u>이 발생할 가능성 있음
* 이러한 상황에서 댓글 수 증가에 따라 **정렬 비용이 주요 병목**이 될 것으로 예상됨
* 회원 정보 JOIN이 부차적인 비용 증가 요인이 될 수 있으나, PK 기반 조회 구조로 인해 영향이 제한적일 것으로 판단하여 본 실험에서는 정렬 및 스캔 구조에 집중
...
* **테스트 데이터**
    ...
    * 댓글 작성자는 3명으로 편중 (PK 기반 JOIN 비용 최소화)
* **측정 횟수**
    * 워밍업 구간(첫 2회 실행값) 제거 -> 초기 DB 캐싱 및 오버헤드의 영향을 제거
    * 이후 10번 반복 측정
...
-> Seq Scan on site_member_prof  (cost=0.00..1.03 rows=3 width=532) (actual time=0.004..0.004 rows=1 loops=10000)
...
### 📍 인덱스 적용 판단 & 근거
* **판단 과정**
  * 게시글 기준 필터링(WHERE) + 최신순 정렬(ORDER BY)이 동시에 발생하는 쿼리 구조 => 필터링 + 정렬의 동시 최적화 필요
  * 하나의 게시글에 댓글 10,000건이 집중된 상황 => 필터링 전용 인덱스 필요 가능성 존재

* **인덱스 설계 근거**
  * <u>복합 인덱스(post_ulid, created_at):</u> 게시글 필터링 + 최신순 정렬 동시 수행 -> 가장 효율성이 뛰어나리라 예상함
  * <u>단일 인덱스(post_ulid):</u> 단일 게시글에 댓글이 집중된 상황 -> DB 옵티마이저가 필터링 전용 인덱스 사용 가능성이 있다고 판단
  * 단일 인덱스(created_at): 전체 댓글 중 최신순으로 읽으며 게시글 필터링을 대조해야 함 -> 비효율적이라 예상 + 도입 고려 X
...
-- WHERE절(post_ulid) 및 ORDER BY절(created_at) 최적화 용도
-- leading column인 post_ulid를 좌측에 배치
...
### 정렬
* 복합 인덱스의 순서가 ORDER BY를 만족하며 정렬 자체가 최적화되었거나 정렬 단계의 비용이 감소되었다고 해석됨
* 복합 인덱스를 사용했음에도 Sort가 제거되지 않은 이유는 JOIN 이후 결과에 추가 정렬이 필요했기 때문으로 추정
...
## 💡 실무 환경과의 연관성
* 현재 API는 별도의 페이지네이션 처리 없이 특정 게시글의 댓글을 전체 조회한 뒤 FE에서 무한 스크롤 형태로 소비하는 구조 -> 댓글 수가 증가할 경우 조회 시점의 부하가 커질 가능성이 있음
* 일반적으로 댓글 조회에 cursor 기반 pagination을 통해 조회 범위를 제한하는 방식이 많이 사용됨
* (post_ulid, created_at) 복합 인덱스는 cursor 기반 조회 패턴(특정 post_ulid + created_at 범위 조건)과 잘 맞으며, 정렬과 탐색을 동시에 최적화 가능함
* **결론:** 현재 구조뿐만 아니라 향후 페이지네이션이 도입되더라도 해당 인덱스는 그대로 효율적으로 활용될 수 있음
