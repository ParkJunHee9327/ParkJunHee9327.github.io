...
"클래스가 변경되어야 할 이유는 오직 하나여야 한다"는 원칙이다. 즉 클래스는 하나의 책임만 져야 한다는 것이다. </p>
<p>

책임이란 "단일한 이해관계자 또는 비즈니스 규율"을 의미한다. (결제 부서는 결제만, 고객 관리 부서는 고객 관리만 함) </p>
...
public class Employee {
    ...
    public double calculatePay() {
        // 근무 시간 + 보너스 + 기여도 등을 계산하는 로직...
        return salary * 0.8;
    }

    // 책임 2: 데이터 영속화
    public void saveToDatabase() {
        // DB와 연결하고 근로자 데이터를 저장하는 로직...
        System.out.println("Saving " + name + " to database...");
    }

    // 책임 3: 보고서 생성
    public void generatePerformanceReport() {
        // 보고서 형식을 결정하고 생성하는 로직...
...
"소프트웨어 엔티티는 변화에 개방적이지만 수정에는 폐쇄적이어야 한다"는 원칙이다. </p>
...
> *자식은 부모의 역할을 할 수 있어야 한다!* <br>

<p>

"수퍼 클래스의 객체는 애플리케이션에 손상을 가하지 않고 자신의 서브 클래스로 치환될 수 있어야 한다"는 원칙이다. </p>
...
"어떤 클라이언트라도 자신이 사용하지 않는 메서드에 의존해서는 안 된다"는 원칙이다. </p>
